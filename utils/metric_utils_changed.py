...
def export_results(
    ...
    for batch_idx in range(input_data.image.size(0)):
        ...
        # print(f"Available keys in input_data: {input_data.keys()}")
...
def _save_images(result, batch_idx, out_dir):
    ...
    # Save input images as a 4x6 grid (pads with black if fewer than 24)
    imgs = result.input.image[batch_idx].detach().cpu()  # [v, c, h, w]
    v, c, h, w = imgs.shape
    rows, cols = 4,4
    cells = rows * cols

    # Convert to numpy HWC uint8
    imgs_np = (imgs.numpy() * 255.0).clip(0.0, 255.0).astype(np.uint8)
    imgs_np = np.transpose(imgs_np, (0, 2, 3, 1))  # v, h, w, c

    # Pad or crop to exactly rows*cols
    if v < cells:
        pad_count = cells - v
        pad_img = np.zeros((pad_count, h, w, c), dtype=np.uint8)
        imgs_np = np.concatenate([imgs_np, pad_img], axis=0)
    elif v > cells:
        imgs_np = imgs_np[:cells]
    ...
    for idx in range(cells):
        r = idx // cols
        col = idx % cols
        canvas[r * h : (r + 1) * h, col * w : (col + 1) * w, :] = imgs_np[idx]

    Image.fromarray(canvas).save(os.path.join(out_dir, "input_grid_4x4.png"))
